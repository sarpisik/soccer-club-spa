...
const BaseShowCompetition = ({ data, next, scrollRef }) =>
  data.map(competition => (
    <div
      ref={scrollRef}
      key={competition.date}
      className="border-primary col-sm-5">
      ...
      <Col sm={4}>
        ...
        <p className="text-center">{competition.home}</p>
      </Col>
      <Col sm={4}>
        <Col sm={12} className="text-center score-board">
          {(next && 'VS') ||
            `${competition.homeGoal} - ${competition.awayGoal}`}
        </Col>
      </Col>
      <Col sm={4}>
        <div className="match-team">
        ...
        <p className="text-center">{competition.away}</p>
      </Col>
      <Col sm={12} className="text-center">
...
const Schedule = ({ data, match }) => {
  ...
  return (
    <div
      ...
      <Grid>
        <Row>
          <ShowCompetition next data={nextMatch} match={match} />
          <ShowCompetition data={lastMatch} match={match} />
        </Row>
      </Grid>
    </div>
  )
